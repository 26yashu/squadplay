  ...
  return (
    <ScreenWrapper className="pb-4 flex flex-col overflow-y-auto">
      ...
      {mode === 'individual' ? (
            ...
            >
              <Card className={`p-5 flex flex-col border-2 ${i === 0 ? 'border-amber-warning bg-amber-warning/10' : 'border-white/5 bg-black/40'}`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <ResultsCard title="Accuracy" value={`${r.stats.accuracy}%`} />
                  <ResultsCard title="Correct" value={r.stats.correct} />
        ...
        <div className="flex flex-col gap-6 mb-8">
          <Card className="p-6 border-neon-indigo/50 bg-neon-indigo/10 text-center">
            <h2 className="text-gray-400 mb-2">Squad Score</h2>
            <div className="text-6xl font-black text-white mb-6 drop-shadow-[0_0_15px_rgba(99,102,241,0.5)]">
              {results.stats.score}
            </div>
            <div className="grid grid-cols-2 gap-4">
              <ResultsCard title="Accuracy" value={`${results.stats.accuracy}%`} />
              <ResultsCard title="Correct" value={results.stats.correct} subtitle={`${results.stats.wrong} Wrong`} />
            </div>
          </Card>
        </div>
      )}
