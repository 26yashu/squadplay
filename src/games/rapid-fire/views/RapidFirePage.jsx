...
import { RapidQuestionCard } from '../components/RapidQuestionCard';
import { OptionButton } from '../../quiz-battle/components/OptionButton';
import { CountdownTimer } from '../components/CountdownTimer';
import { RapidScoreBoard } from '../components/RapidScoreBoard';
...
export function RapidFirePage() {
  ...
  const gameConfig = getGameById('rapid-fire');
  const navigate = useNavigate();
  
  const game = useRapidFireGame(session, players);
  ...
  if (game.state === 'player_transition') {
    return (
      <ScreenWrapper className="flex flex-col items-center justify-center text-center">
        <h2 className="text-4xl font-black mb-4 text-orange-500">Next Up</h2>
        ...
        <Button onClick={game.start} className="w-full max-w-sm py-4 text-lg bg-orange-500 hover:bg-orange-600 text-black">I'm Ready</Button>
      </ScreenWrapper>
    );
  }
  ...
  const handleOptionClick = (option) => {
    ...
    setTimeout(() => {
      game.nextQuestion();
    }, 500); // 500ms delay for rapid fire
  };

  const currentStats = session.mode === 'individual' 
    ? game.liveStats?.[game.currentPlayer?.id]
    : game.liveStats?.['squad'];
  ...
  return (
    <ScreenWrapper className="pb-4 flex flex-col">
      ...
      <RapidScoreBoard player={game.currentPlayer} stats={currentStats} mode={session.mode} />
      ...
      <div className="flex-1 flex flex-col relative min-h-0">
        <AnimatePresence mode="wait">
          {game.currentQuestion && (
            ...
            >
              <RapidQuestionCard question={game.currentQuestion.question} />
